...
const resolvers = {
  Query: {
    ...
    kpis: (_, { range }) => {
      ...
      return Array.from({ length: days }, (_, i) => ({
        stock: Math.floor(Math.random() * 200 + 100), // Mock stable-ish data
  ...
  Mutation: {
    ...
    transferStock: (_, { id, from, to, qty }) => {
      ...
      if (!productFrom) throw new Error('Product not found in from warehouse');
      if (productFrom.stock < qty) throw new Error('Insufficient stock');
